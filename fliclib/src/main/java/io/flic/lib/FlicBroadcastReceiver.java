 ...
 */
public abstract class FlicBroadcastReceiver extends BroadcastReceiver implements FlicIntentValues {
  ...
  public final void onReceive(@NonNull final Context context, final Intent intent) {
    ...
    FlicManager.getInstance(context, new FlicManagerInitializedCallback() {
      @Override
      public void onInitialized(@NonNull final FlicManager manager) {
        if (!manager.validateIntent(intent)) {
          Log.d(TAG, "Invalid intent received");
          return;
        }

        final String deviceId = intent.getStringExtra(FlicIntentExtras.BUTTON_ID);
        FlicButton button = manager.getButtonByDeviceId(deviceId);
        if (button == null) { // For removed
          button = new FlicButton(manager, deviceId);
        }

        final String value = intent.getStringExtra(FlicIntentExtras.VALUE);
        final boolean wasQueued = intent.getBooleanExtra(FlicIntentExtras.WAS_QUEUED, false);
        final int timeDiff = intent.getIntExtra(FlicIntentExtras.TIME_DIFF, 0);

        switch (intent.getStringExtra(FlicIntentExtras.TYPE)) {
          case FlicIntentTypes.UP_OR_DOWN:
            onButtonUpOrDown(context, button, wasQueued, timeDiff, UP.equals(value), DOWN.equals(value));
            break;

          case FlicIntentTypes.CLICK_OR_HOLD:
            onButtonClickOrHold(context, button, wasQueued, timeDiff, CLICK.equals(value), HOLD.equals(value));
            break;

          case FlicIntentTypes.SINGLE_OR_DOUBLE_CLICK:
            onButtonSingleOrDoubleClick(context, button, wasQueued, timeDiff, SINGLE_CLICK.equals(value), DOUBLE_CLICK.equals(value));
            break;

          case FlicIntentTypes.SINGLE_OR_DOUBLE_CLICK_OR_HOLD:
            onButtonSingleOrDoubleClickOrHold(context, button, wasQueued, timeDiff, SINGLE_CLICK.equals(value), DOUBLE_CLICK.equals(value), HOLD.equals(value));
            break;
   ...
   */
  protected abstract void onRequestAppCredentials(@NonNull final Context context);
   ...
   */
  public void onButtonUpOrDown(@NonNull final Context context, @NonNull final FlicButton button, boolean wasQueued, int timeDiff, boolean isUp, boolean isDown) {
  }
   ...
   */
  public void onButtonClickOrHold(@NonNull final Context context, @NonNull final FlicButton button, boolean wasQueued, int timeDiff, boolean isClick, boolean isHold) {
  }
   ...
   */
  public void onButtonSingleOrDoubleClick(@NonNull final Context context, @NonNull final FlicButton button, boolean wasQueued, int timeDiff, boolean isSingleClick, boolean isDoubleClick) {
  }
   ...
   * @param isHold        True if hold, else false
   */
  public void onButtonSingleOrDoubleClickOrHold(@NonNull final Context context, @NonNull final FlicButton button, boolean wasQueued, int timeDiff, boolean isSingleClick, boolean isDoubleClick, boolean isHold) {
  }
   ...
   * @param button  The button
   */
  public void onButtonRemoved(@NonNull final Context context, @NonNull final FlicButton button) {
  }
}
